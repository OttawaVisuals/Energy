...
**Confirmed the border pattern is GFEI's own allocation, not the projection (2026-08-12).** The map visibly shows sharp edges that trace provincial borders in the background/shared-rate cells. Checked directly rather than assumed: built each province's boundary polygon from the same `geo_json/*.json` files `canada_boundary.py` already unions for the Canada mask, then for each of GFEI's most-repeated shared gas values, checked which province every cell holding that value falls in. Every top repeated value is ≥99% contained within one province (10,584/10,620 cells at 8.816e-10 → NT; 4,499/4,501 at 1.159e-06 → SK; similarly clean single-province splits for ON, QC, MB, BC), with the handful of stragglers explained by cells on a shared border or by the province polygons' own simplification tolerance, not by a block genuinely spanning two provinces. Confirms the mechanism already documented above: GFEI builds its background/proxy rate from countries' own reports where available, and Canada's National Inventory Report breaks fugitive emissions down by province, so a province becomes the unresolved area a shared rate gets spread across. The Albers projection is a smooth continuous transform and can't produce a border-shaped edge on its own, so this rules out the map draw as the source. Documented on-page in both the near-map caveat and the `#m-methane-map` methodology entry; no pipeline change needed since "Hide background" already excludes every cell carrying a shared value (province-block artifacts included) via `bg_bits` — the fix here is explanatory, not corrective.
