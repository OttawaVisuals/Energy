...
MEASURES = [
    ...
    ('Windows_Partial',              'Some windows replaced'),
...
def compute_slice(df):
    ...
    # ---- GHG (mirrors renderGHG()) ----
    # One basis since 2026-09-23: each home's own fuel consumption x current
    # (2026) official ECCC/OBPS factors, from Python/compute_ghg_scenarios.py
    # (~100% coverage). The raw ERSGHG field is not used: the EnerGuide data
    # team advised against it (reported for only ~50% of pairs, factors
    # updated sporadically).
    pair = pd.DataFrame({'pre': num(df.get('Pre_GHG_current')),
                         'post': num(df.get('Post_GHG_current'))}).dropna()
    pre_v = pair['pre'].to_numpy()
    post_v = pair['post'].to_numpy()
    ghg_pre_med = median(pre_v)
    ghg_post_med = median(post_v)
    ...
    out['ghg_pre_bins'] = bin_counts(pre_v, step=1, max_val=30)
    out['ghg_post_bins'] = bin_counts(post_v, step=1, max_val=30)
    out['ghg_delta_bins'] = bin_counts(deltas, step=1)
