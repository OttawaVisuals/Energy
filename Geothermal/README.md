...
first is built. `GridCapacity/ottawa_capacity.geojson` and the
`GridCapacity/Hydro.py` fetch script that produces it are both absent from
this checkout — and `Hydro.py` was never committed to `main` (the whole
`GridCapacity/` directory is gitignored, unlike every other pipeline script in
the repo), so it cannot be regenerated by re-running anything currently
checked in. `build_suitability.py`'s own `feeder` factor is unrunnable for the
same reason (`[warn] ottawa_capacity.geojson missing -> factor all-zero`,
unchanged by this session). Producing `feeder_demand.geojson` and its
coincidence factor is unstarted — see `ROADMAP.md` item 7.
...
needed. `feeder` factor is unchanged (`all-zero`, blocked as above). No prior
