...
### 3.13 `build_heat_demand.py` — grid aggregation (Heat Demand Phase 4, grid half)

**Scope: grid half only.** Phase 4 has two deliverables (`HEATDEMAND_PLAN.md`
§4): `heat_demand_grid.geojson` (500 m cells) and `feeder_demand.geojson`
(Hydro Ottawa feeder polygons + a coincidence-factor stress metric). Only the
first is built. At the time of writing, `GridCapacity/ottawa_capacity.geojson`
and the `GridCapacity/Hydro.py` fetch script that produces it were both absent
from this checkout, and `Hydro.py` had never been committed to `main` (the
whole `GridCapacity/` directory was gitignored, unlike every other pipeline
script in the repo), so `build_suitability.py`'s own `feeder` factor was
unrunnable too (`[warn] ottawa_capacity.geojson missing -> factor all-zero`).
**Both were recovered 2026-08-03** from the 2026-07-27 `gh-pages` deploy
snapshot — the only place `Hydro.py` had ever existed — and `Hydro.py` is now
committed to `main`, with `.gitignore` narrowed to exclude only the
directory's generated contents. Re-run `build_suitability.py` to pick the
`feeder` factor up. Note `Hydro.py` is the original working script and has not
been tidied: its `WHERE` clause matches every LDC whose name contains
"Ottawa" rather than a verified list, and it writes its output to the current
directory rather than into `GridCapacity/` — confirm the LDC filter before
using it to refresh. Producing `feeder_demand.geojson` and its coincidence
factor is still unstarted — see `ROADMAP.md` item 7 and
`GEOTHERMAL_STATUS.md`'s 2026-08-03 entry.

**What it does.** Aggregates `buildings_ottawa.parquet` (Phases 1–3) onto the
canonical 500 m grid (`thermal_conductivity_grid.geojson`, 13,778 cells,
`idw.py` conventions), restricted to `in_ottawa_cd == True` — Phase 2.5's
established convention, and also where `grid_cell_id` is 100% populated (the
15% of buildings outside the CD are where the null `grid_cell_id`s
concentrate — 85% of them — so this scope decision sidesteps that gap rather
than papering over it). 6,722 of the 13,778 cells (49%) contain at least one
building; the rest are emitted as absent, not zero — no invented demand in
empty cells.

Per cell: `kwh_yr` (current annual space-heat, all fuels — the exact field
name `build_suitability.py`'s `f_from_polys` already expects), `design_kw`,
`elec_kw_peak_now` (current electric-heated stock only), the three Phase 3
electrification policies' annual kWh + design-day kW
(`elec_kwh_electrified`/`elec_kw_peak_electrified`,
`..._hybrid`, `..._gshp`), a reserved always-null `waste_heat_kwh`, modal
`dominant_fuel`, `n_buildings`/`n_heated`, and a `class_counts` JSON dict.

**Validated:** every summed column's cell-level total equals the
building-level total exactly (assert, not eyeball). Independently cross-checks
clean against both upstream phases without adjustment: residential-only
subset **6.68 TWh/yr** vs Phase 2.5's reported 6.68 TWh; policy (a)/(b)/GSHP
added load **4.61 TWh / 5,264 MW**, **3.41 TWh / 0 MW**, **2.51 TWh / 1,482
MW** vs Phase 3's reported +4,613 GWh/+5,264 MW, +3,410 GWh/+0 MW, +2,515
GWh/+1,482 MW. (All-classes total is 12.20 TWh/yr — commercial/institutional/
industrial roughly double the residential-only figure; don't compare that
number to Phase 2.5's residential-only 6.68 TWh.)

**`*_kw_peak_*` columns are still undiversified**, unchanged from Phase 3 —
summing them per cell does not turn them into a load-flow result, only into a
larger undiversified sum. Treat them as an upper bound, not MW headroom,
until the (currently unstarted) coincidence factor exists.

**Reran `build_suitability.py`** with the new layer present: the `demand`
factor now reads real per-cell `kwh_yr` (`mean 0.08, >0 in 6,199/13,778
cells`) instead of the City-serviced-area binary proxy — the upgrade hook
documented in §3.9 fires automatically once the file exists, no code change
needed. `feeder` factor was unchanged in that run (`all-zero`) for the reason
above; the geojson it needs was recovered 2026-08-03, so a re-run now scores
it. No prior
`suitability_grid.geojson` build was preserved to diff score-by-score; the
factor-level before/after is the demand proxy (binary, `>0` in all 8%-serviced
cells) vs the real intensity (continuous, `>0` in 45% of cells) — a
qualitatively finer demand signal for the `district` segment (demand weight
.35), though the `district` composite score range and correlations
(`res↔district −0.068`, `large↔district −0.106`) look unchanged in shape.

Usage: `python Geothermal/scripts/build_heat_demand.py`
